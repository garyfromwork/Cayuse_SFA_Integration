...
ACADEMIC_SU AS (
    SELECT DISTINCT 'DEANS/CHAIRS' TITLE, RAW_ACADEMIC_SU.* FROM RAW_ACADEMIC_SU
...
RAW_FACULTY AS (
    ...
    AND (UPPER(NBRJOBS_DESC) NOT LIKE '%ADJUNCT%' 
        ...
        AND UPPER(NBRJOBS_DESC) NOT LIKE UPPER('ASST TO PRESIDENT')
        AND UPPER(NBRJOBS_DESC) NOT LIKE UPPER('ASST TO DEAN STDNTS')
        AND UPPER(NBRJOBS_DESC) NOT LIKE UPPER('SPP%')
        AND UPPER(NBRJOBS_DESC) NOT LIKE UPPER('%TITLE IX%'))
),
FACULTY AS (
    SELECT DISTINCT 'FACULTY' AS TITLE, RAW_FACULTY.* FROM RAW_FACULTY
...
EMPLOYEE AS (
    SELECT DISTINCT 'EMPLOYEE' AS TITLE, RAW_EMPLOYEE.* FROM RAW_EMPLOYEE
...
EXECUTIVE_ASSISTANTS AS (
    SELECT DISTINCT 'EXECUTIVE ASSISTANTS' TITLE, RAW_EXECUTIVE_ASSISTANTS.* FROM RAW_EXECUTIVE_ASSISTANTS
...
ASSISTANTS AS (
    SELECT DISTINCT 'DEAN ASSISTANTS' TITLE, RAW_ASSISTANTS.* FROM RAW_ASSISTANTS
...
ADMIN AS (
    SELECT DISTINCT 'ADMIN ASSISTANTS' TITLE, RAW_ADMIN.* FROM RAW_ADMIN
...
SELECT DISTINCT NVL((SELECT distinct GOBTPAC_EXTERNAL_USER FROM gobtpac WHERE GOBTPAC_PIDM IN (SELECT SPRIDEN_PIDM FROM SPRIDEN WHERE SPRIDEN_ID = POSNS.SPRIDEN_ID AND SPRIDEN_CHANGE_IND IS NULL)), ' ') Username,
       POSNS.SPRIDEN_ID EmployeeID,
       (SELECT distinct FTVORGN_FUND_CODE_DEF || '-' || POSNS.nbrjobs_orgn_code_ts 
        FROM FTVORGN 
